...
- 消息：私聊、群聊、应用会话、话题、回复、转发、回应、选择消息、结构化链接、附件、移动端相册多选/相机图片预览、说明及 Android 进程恢复、桌面文件拖放与截图、语音、Markdown 编辑与渲染、图表与系统事件
...
- 桌面集成：Windows、macOS、Linux 完整包应用内更新、系统托盘、开机静默自启动、未读会话快捷入口、区域/全屏截图、全局快捷键，以及关闭窗口后后台运行/退出应用设置
...
推送与 `pubspec.yaml` 版本一致的 `v*` 标签后，Release 工作流会使用固定 Flutter 3.47.2 构建并发布完整的跨平台产物：Android 通用/分 ABI APK 与 AAB、未签名 iOS IPA、macOS、Windows x64、Linux x64/arm64、Web 压缩包及 SHA-256 校验文件。发布前会验证预期产物集合完整。Android 发布产物使用 GitHub Actions Secrets 中保存的固定 Release 证书，工作流会对照 [`android/release-signing-certificate.sha256`](android/release-signing-certificate.sha256) 校验所有 APK 与 AAB；iOS 产物仍需要使用自己的 Apple 开发者证书重新签名。
