...
void main() {
  test('默认使用 release 更新源', () {
    ...
    expect(UpdateService.currentVersion, '0.3.22');
    expect(UpdateService.currentBuild, 35);
  });

  test('只接受 HTTPS 下载地址并识别新版本', () async {
    final client = MockClient((request) async => http.Response(
        '{"android":{"version":"0.3.23","build":36,"url":"https://example.com/app.apk"}}',
        200));
    final release = await UpdateService(client: client).check();
    expect(release?.build, 36);
  ...
  test('按平台选择版本清单并保留对应下载地址', () async {
    final client = MockClient((request) async => http.Response(
        ...
        '"ios":{"version":"0.3.23","build":36,"url":"https://example.com/app.ipa"}}',
        200));
    final release =
        await UpdateService(client: client, platform: AppUpdatePlatform.ios)
            .check();
    expect(release?.version, '0.3.23');
    expect(release?.build, 36);
  ...
  test('桌面端从 GitHub Release 选择对应平台产物', () async {
    final client = MockClient((request) async {
      ...
      return http.Response(
          '{"tag_name":"v0.3.23","assets":[{"name":"MagicChat-Windows-x64.zip",'
          '"size":1234,"browser_download_url":"https://github.com/techblack/MagicChat-Flutter/releases/download/v0.3.23/MagicChat-Windows-x64.zip"},'
          '{"name":"SHA256SUMS.txt","browser_download_url":"https://github.com/techblack/MagicChat-Flutter/releases/download/v0.3.23/SHA256SUMS.txt"}]}',
          200);
    });
    final release =
  ...
  test('桌面端没有对应产物时拒绝响应', () async {
    final client = MockClient(
        (_) async => http.Response('{"tag_name":"v0.3.23","assets":[]}', 200));
    expect(
