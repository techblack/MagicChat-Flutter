...
| 多账户 | 设置页、`SessionStore` | 安全存储多个 Server 账户、切换和长按删除、会话失效后保留账户并标记需重新登录、需重新登录账户的密码认证 UI 已接入 |
...
| 私聊、群聊、应用会话 | 消息 → 会话 → 消息面板 | 私聊/群聊创建、应用会话打开、联系人打开会话、公开群组加入、成员多选/添加/移除、群头像/公告/名称/公开状态修改、会话移除/恢复、置顶/免打扰、会话关键词搜索与全部/未读/私聊/群聊/应用筛选、按置顶和最新时间稳定排序、会话标题居中、安全区适配、双击消息选择复制、左滑引用、群成员资料卡、Enter/Ctrl-Command+Enter 发送设置、图片比例气泡、内容自适应气泡和顶部历史加载反馈已接入；话题创建、列表/详情/参与/关闭、来源消息上下文与话题回复预览、消息转发、消息面板和历史分页已接入；归档话题的发送、回复、回应、选择和撤回入口已按原版门控；群成员角色权限门控、退出/解散已接入；普通用户设置管理员暂缺服务端 API。 |
...
- 账户：邮箱验证码注销账号、完整 Server 管理页。
- 会话：独立聊天详情页、私聊转群聊、图片消息画廊、乐观发送及送达状态、智能语音输入。
