...
| 综合搜索 | AppBar 搜索、`features/search` | 会话、联系人、项目复用现有列表接口，聊天记录调用 `/api/client/search/messages`；结果分类展示并支持跳转对应会话或主导航页 |
...
| 推送授权与通知路由 | `PushService`、`PushTokenProvider`、`PushRegistrationStore`、设置页、`AppShell` | 公共 Push Gateway installation/grant 注册、续期、持久化和撤销，私有 Server grant 注册/撤销、启动路由解析、通知总开关持久化、开启通知时系统权限请求、Android/iOS 本地通知通道及原生令牌桥接契约已接入；Android JPush SDK 可通过 `JPUSH_APP_KEY` 构建参数启用，Android/iOS 通知点击路由和 iOS APNs 注册已接入，厂商渠道参数与推送凭据仍需按发布环境配置 |
