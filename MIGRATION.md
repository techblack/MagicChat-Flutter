...
| 图片、文件、语音、选择、对象卡片、图表、回应、系统事件 | `_MessageBubble`、`flutter_markdown_plus`、文件选择器、`VoiceRecorder`、`_ChartPreview` | 文件选择与 multipart 发送、Markdown 渲染及链接、图片/音频按 MIME 分流、图片交互预览、附件打开/语音播放入口、历史附件分页列表、麦克风录制发送、类型摘要、折线/柱状/饼图/雷达图专用预览、对象展开、图标、choice 选项响应、reaction 胶囊及长按参与者列表、系统事件摘要和居中展示已接入；撤回消息在历史与实时路径统一显示占位并禁用消息操作；文本和附件回复引用、输入表情选择已接入 |
