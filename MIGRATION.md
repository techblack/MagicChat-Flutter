...
| 本地存储 | 设置页、`StorageService`、`MessageCacheStore` | Native 消息缓存使用 SQLite WAL，并按私聊/群聊/应用/话题分库、按 Server/账户隔离，支持增量 upsert、分页和组合检索；Web 使用串行化持久存储回退；缓存占用统计、路径展示和清理已接入 |
