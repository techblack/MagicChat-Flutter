...
| 现有能力 | Flutter 入口 | 状态 |
| --- | --- | --- |
| 登录、Server 会话 | 启动登录页、`AuthService`、`ServerStore`、`SessionStore`、设置页 | 密码/邮箱验证码登录、登录前用户协议与隐私政策主动同意、Native HTTP/Bearer 与 Web HttpOnly Cookie、退出登录、服务器切换、实时连接清理、会话过期自动返回登录页、邮箱验证码注销账号和主题持久化已接入；内置官方服务器与自定义服务器列表支持迁移、名称/地址编辑、地址去重、选择、最近使用和删除，登录页与设置页均可进入；错误验证码保留会话，成功后删除当前本地凭据并切换到最近可用账号，不确定错误转为重新认证 |
...
| 推送授权与通知路由 | `PushService`、`PushTokenProvider`、`PushRegistrationStore`、设置页、`AppShell` | 公共 Push Gateway installation/grant 注册、续期、持久化和撤销，私有 Server grant 注册/撤销、启动及前台通知点击路由、通知总开关、设备级新消息提示音开关、Android/iOS 本地通知点击会话、Android/iOS/macOS 系统角标及所有平台应用内未读红点已接入；Android JPush SDK 可通过 `JPUSH_APP_KEY` 构建参数启用，厂商渠道参数与推送凭据仍需按发布环境配置 |
