...
| 现有能力 | Flutter 入口 | 状态 |
| --- | --- | --- |
| 登录、Server 会话 | 启动登录页、`AuthService`、`SessionStore`、设置页 | 密码/邮箱验证码登录、Native HTTP/Bearer 与 Web HttpOnly Cookie、退出登录、服务器切换、实时连接清理、会话过期自动返回登录页、邮箱验证码注销账号和主题持久化已接入；错误验证码保留会话，成功后删除当前本地凭据并切换到最近可用账号，不确定错误转为重新认证 |
